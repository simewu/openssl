...
os.system(myCmd)
myCmd = f'{openssl_dir}/apps/openssl genpkey -algorithm rsa -out {openssl_dir}/CA-project/csr/key_srv.key -pkeyopt rsa_keygen_bits:4096 > /dev/null 2>&1'
os.system(myCmd)
myCmd = f'{openssl_dir}/apps/openssl req -new -key {openssl_dir}/CA-project/csr/key_srv.key -out {openssl_dir}/CA-project/csr/key_srv.csr -nodes -pkeyopt rsa_keygen_bits:4096 -subj \'/CN=oqstest server\' -config {openssl_dir}/apps/openssl.cnf > /dev/null 2>&1'
os.system(myCmd)
