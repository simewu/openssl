...
def genKey(algorithm, num_samples, bits = ''):
	if algorithm== 'rsa':
		myCmd = f'{openssl_dir}/apps/openssl genpkey -algorithmsrsa -out {openssl_dir}/CA-project/csr/key_srv.key -pkeyopt rsa_keygen_bits:{bits} > /dev/null 2>&1'
		for i in range (num_samples):
			os.system(myCmd)
	else:
		myCmd = f'{openssl_dir}/apps/openssl genpkey -algorithms{algorithm} -out {openssl_dir}/CA-project/csr/key_srv.key > /dev/null 2>&1'
		for i in range (num_samples):
			os.system(myCmd)
