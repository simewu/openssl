...
def genKey(algorithm, num_samples, bits = ''):
	if algorithm == 'rsa':
		for i in range (num_samples):
			myCmd = f'{openssl_dir}/apps/openssl genpkey -algorithm rsa -out {openssl_dir}/CA-project/csr/key_srv{i}_{algorithm}{bits}.key -pkeyopt rsa_keygen_bits:{bits} > /dev/null 2>&1'
			os.system(myCmd)
	else:
		for i in range (num_samples):
			myCmd = f'{openssl_dir}/apps/openssl genpkey -algorithm {algorithm} -out {openssl_dir}/CA-project/csr/key_srv{i}_{algorithm}{bits}.key > /dev/null 2>&1'
			os.system(myCmd)

def genCSR(algorithm, num_samples, bits = ''):
	if algorithm == 'rsa':
		for i in range (num_samples):
			myCmd = f'{openssl_dir}/apps/openssl req -new -key {openssl_dir}/CA-project/csr/key_srv{i}_{algorithm}{bits}.key -out {openssl_dir}/CA-project/csr/key_srv{i}_{algorithm}{bits}.csr -nodes -pkeyopt rsa_keygen_bits:{bits} -subj \'/CN=oqstest server\' -config {openssl_dir}/apps/openssl.cnf > /dev/null 2>&1'
			os.system(myCmd)
	else:
		for i in range (num_samples):
			myCmd = f'{openssl_dir}/apps/openssl req -new -key {openssl_dir}/CA-project/csr/key_srv{i}_{algorithm}{bits}.key -out {openssl_dir}/CA-project/csr/key_srv{i}_{algorithm}{bits}.csr -nodes -subj \'/CN=oqstest server\' -config {openssl_dir}/apps/openssl.cnf > /dev/null 2>&1'
			os.system(myCmd)

def genCert(algorithm, num_samples, bits =''):
	for i in range (num_samples):
		myCmd = f'{openssl_dir}/apps/openssl x509 -req -in {openssl_dir}/CA-project/csr/key_srv{i}_{algorithm}{bits}.csr -out {openssl_dir}/CA-project/crt/key_crt{i}_{algorithm}{bits}.pem -CA key_CA_{algorithm}{bits}.pem -CAkey key_CA_{algorithm}{bits}.key -CAcreateserial -days 365' 
		os.system(myCmd)

def certVerify(algorithm, num_samples, bits = ''):
	for i in range (num_samples):
		myCmd = f'{openssl_dir}/apps/openssl verify -CAfile {openssl_dir}/CA-project/key_CA_{algorithm}{bits}.pem {openssl_dir}/CA-project/crt/key_crt{i}_{algorithm}{bits}.pem {openssl_dir}/CA-project/crt/key_crt{i}_{algorithm}{bits}.pem'
		os.system(myCmd)



algorithms = ['rsa 2048', 'rsa 3072', 'rsa 4096', 'dilithium2', 'dilithium3', 'dilithium4', 'falcon512', 'falcon1024', 'rsa3072_dilithium2', 'rsa3072_dilithium3', 'rsa3072_falcon512', 'p256_dilithium2', 'p256_dilithium3', 'p384_dilithium4', 'p256_falcon512'] #, 'p512_falcon512']
	...
	for algorithm in algorithms:
		...
		if algorithm == 'rsa':
			for bits in rsa_bits_array:
				...
				t2 = time.time()
				avg_cert_signing_request_time = t2 - t1
...
fileName = 'LOGGED_OPENSSL.csv'
filePath = os.path.join(f'{openssl_dir}/CA-project',fileName)
