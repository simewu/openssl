...
def initCert(algorithm, bits = ''):
	if algorithm == 'rsa':
		...
		os.system(myCmd)
	if algorithm == 'secp':
		myCmd =f'{openssl_dir}/apps/openssl ecparam -out key_CA_{algorithm}{bits}.key -name secp521r1 -genkey '
		os.system(myCmd)
...
def genCSR(algorithm, num_samples, bits = ''):
	if algorithm == 'rsa':
		...
		for i in range (num_samples):
			os.system(myCmd)
	if algorithm == 'secp':
		myCmd =f'{openssl_dir}/apps/openssl req -newkey ec:key_CA_{algorithm}{bits}.key -keyout ec_PRIVATEKEY.key -out key_srv_{algorithm}{bits}.csr -nodes -subj \'/CN=oqstest server\' -config {openssl_dir}/apps/openssl.cnf'
		for i in range (num_samples):
			os.system(myCmd)
	else:
...
def genCert(algorithm, num_samples, bits =''):
	...
	if algorithm == 'secp':
		myCmd=f'{openssl_dir}/apps/openssl x509 -req -in key_srv_{algorithm}{bits}.csr -out key_crt_{algorithm}{bits}.pem -CA key_CA{algorithm}{bits}.pem -CAkey key_CA.key -CAcreateserial -days 365'
		for i in range (num_samples):
			os.system(myCmd)
	else:
...
def certVerify(algorithm, num_samples, bits = ''):
	if algorithm == 'secp':
		myCmd = f'{openssl_dir}/apps/openssl verify -CAfile key_CA_{algorithm}{bits}.pem key_crt_{algorithm}{bits}.pem key_crt_{algorithm}{bits}.pem'
		for i in range (num_samples):
			os.system(myCmd)
	else:
...
#algorithms = [
#	'rsa','dilithium2','dilithium3','dilithium5','dilithium2_aes','dilithium3_aes','dilithium5_aes', 'falcon512', 'falcon1024', 'rainbowIclassic','rainbowIcircumzenithal','rainbowIcompressed','rainbowIIIclassic','rainbowIIIcircumzenithal','rainbowIIIcompressed','rainbowvclassic','rainbowVcircumzenithal','rainbowVcompressed','rsa3072_dilithium2','rsa3072_falcon512','rsa3072_rainbowIclassic','rsa3072_rainbowIcircumzenithal','rsa3072_rainbowIcompressed','p256_dilithium2','p256_falcon512','p384_dilithium3', 'p521_dilithium5','p521_falcon1024','p256_rainbowIclassic','p256_rainbowIcircumzenithal','p256_rainbowIcompressed','p384_rainbowIIIclassic','p384_rainbowIIIcircumzenithal','p384_rainbowIIIcompressed','p521_rainbowVclassic','p521_rainbowVcircumzenithal','p521_rainbowVcompressed'
#]
#algorithms_in_english = [
#	'RSA', 'Dilithium 2', 'Dilithium 3', 'Dilithium 5', 'Dilithium 2 with AES', 'Dilithium 3 with AES', 'Dilithium 5 with AES', 'Falcon 512', 'Falcon 1024', 'Rainbow I Classic','Rainbow I Circumzenithal','Rainbow I Compressed','Rainbow III Classic','Rainbow III Circumzenithal','Rainbow III Compressed','Rainbow V Classic','Rainbow V Circumzenithal','Rainbow V Compressed', 'RSA 3072 + Dilithium 2','RSA 3072 + Falcon 512', 'RSA 3072 + Rainbow I Classic','RSA 3072 + Rainbow I Circumzenithal','RSA 3072 + Rainbow I Compressed','P256 + Dilithium 2', 'P256 + Falcon 512','P384 + Dilithium 3', 'P521 + Dilithium 5', 'P521 + Falcon 1024','P256 + Rainbow I Classic','P256 + Rainbow I Circumzenithal','P256 + Rainbow I Compressed','P384 + Rainbow III Classic','P384 + Rainbow III Circumzenithal','P384 + Rainbow III Compressed','P521 + Rainbow V Classic','P521 + Rainbow V Circumzenithal','P521 + Rainbow V Compressed'
#]
